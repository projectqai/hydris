// ICAO 24-bit aircraft address country allocations.
// Source: ICAO Annex 10 Vol III, Part I, Chapter 9, Table 9-1
// Reference: naccdca3wp05.pdf (checked into this directory)
...
// Sorted by start address. Smaller (more specific) ranges appear
// before their parent block so the lookup picks the best match.
...
export function lookupCountry(icaoAddr: number): string | undefined {
	let best: HexRange | undefined;
	for (const r of ranges) {
		if (icaoAddr < r.start) continue;
		if (icaoAddr <= r.end) {
