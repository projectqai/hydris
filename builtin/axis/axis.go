...
func Run(ctx context.Context, logger *slog.Logger, _ string) error {
	...
	if err := controller.Push(ctx, controllerName, &pb.Entity{
		Id:    serviceEntityID,
		Label: proto.String("AXIS Cameras"),
			...
			SupportedDeviceClasses: []*pb.DeviceClassOption{
				{
					Class:       "camera",
					Label:       "AXIS Camera",
					Description: "An AXIS-branded IP camera reachable over the network. Hydris connects via the AXIS HTTP API for video streaming and PTZ control.",
				},
			},
		},
	...
	go controller.Run(ctx, controllerName, serviceEntityID, func(ctx context.Context, entity *pb.Entity, ready func()) error { //nolint:errcheck
	...
	classes := []controller.DeviceClass{
		{Class: "camera", Label: "AXIS Camera", Schema: cameraSchema()},
	}
